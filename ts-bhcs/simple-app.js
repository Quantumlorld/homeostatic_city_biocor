...
class SimpleBHCS {
    ...
    applyBioCore() {
        ...
        const synergyInput = document.getElementById('synergy-input');
    ...
    calculateEffect(plant, drug, synergy) {
    ...
    applyEffectToZone(zoneId, effect) {
    ...
    updateMetrics() {
        ...
        if (avgActivityEl)
            avgActivityEl.textContent = avgActivity.toFixed(2);
        if (systemHealthEl)
            systemHealthEl.textContent = `${systemHealth.toFixed(0)}%`;
        if (activeEffectsEl)
            activeEffectsEl.textContent = this.effects.length.toString();
        if (balanceScoreEl)
            balanceScoreEl.textContent = balance.toFixed(2);
    }
